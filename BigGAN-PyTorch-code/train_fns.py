...
def GAN_training_function(G, D, GD, z_, y_, ema, state_dict, config, writer_dict, gamma=10.0):
  def train(x, y):
    G.optim.zero_grad()
    D.optim.zero_grad()
    ...
    for step_index in range(config['num_D_steps']):
      ...
      for accumulation_index in range(config['num_D_accumulations']):
        ...
        #D_reg = eval('losses.d_r1_loss')(D_real, x[counter])
...
# ==========================
# Training function with sparse G or sparse D.
def GAN_training_sparse_function(mask, G, D, GD, z_, y_, ema, state_dict, config, writer_dict):
  def train(x, y):
    G.optim.zero_grad()
    D.optim.zero_grad()
    # How many chunks to split x and y into?
    ...
    for accumulation_index in range(config['num_G_accumulations']):    
      ...
      y_.sample_()
      D_fake, vanilla_fake, cls_fake = GD(z_, y_, train_G=True, split_D=config['split_D'])
      G_loss = eval('losses.' + config['g_loss'])(D_fake) / float(config['num_G_accumulations'])
      G_loss.backward()
    summary = {'G_loss': torch.mean(G_loss),
               'vanilla_fake': torch.mean(vanilla_fake),
               'cls_fake': torch.mean(cls_fake),
...
def GAN_training_function_GbarAug(G, D, G_bar, GD, z_, y_, ema, state_dict, config, writer_dict):
  def train(x, y):
    G.optim.zero_grad()
    D.optim.zero_grad()
    # How many chunks to split x and y into?
    ...
    for step_index in range(config['num_D_steps']):
      ...
      D.optim.zero_grad()
      for accumulation_index in range(config['num_D_accumulations']):
        # Generate random distribution, to determine to use real data or G_bar(z).
        distri = torch.rand(1)
        z_.sample_()
        y_.sample_()
        if distri[0] < 0.5:
          ...
          G_z = G_bar(z_[:config['batch_size']], y_[:config['batch_size']])
          D_pair, vanilla_pair, cls_pair = GD(z_[:config['batch_size']], y_[:config['batch_size']], 
                              G_z[:config['batch_size']], y_[:config['batch_size']], train_G=False, 
                              split_D=config['split_D'])
        D_fake, D_real = D_pair[0], D_pair[1]
        vanilla_fake, vanilla_real = vanilla_pair[0], vanilla_pair[1]
        cls_fake, cls_real = cls_pair[0], cls_pair[1]
...
def GAN_training_compression_function(G, D, GD, z_, y_, ema, state_dict, config, writer_dict, gamma=10.0):
  def train(x, y):
    G.optim.zero_grad()
    D.optim.zero_grad()
    # How many chunks to split x and y into?
...
# DGL tuning, with sparse D.
def GAN_training_compression_sparse_function(mask, G, D, GD, z_, y_, ema, state_dict, config, writer_dict):
  def train(x, y):
    G.optim.zero_grad()
    D.optim.zero_grad()
    # How many chunks to split x and y into?
    ...
    for step_index in range(config['num_D_steps']):
      ...
      for accumulation_index in range(config['num_D_accumulations']):
        ...
        x[counter].requires_grad = True
        #D_pair, vanilla_pair, cls_pair = GD(z_[:config['batch_size']], y_[:config['batch_size']], x[counter], y[counter], train_G=False, split_D=config['split_D'], with_Dbar=False)
        D_pair, vanilla_pair, cls_pair, D_bar_pair, _, _ = GD(z_[:config['batch_size']], y_[:config['batch_size']], x[counter], y[counter], train_G=False, split_D=config['split_D'], with_Dbar=True)
        D_fake, D_real, vanilla_fake, vanilla_real, cls_fake, cls_real = D_pair[0], D_pair[1], vanilla_pair[0], vanilla_pair[1], cls_pair[0], cls_pair[1]
        D_fake_bar, D_real_bar = D_bar_pair[0], D_bar_pair[1]
        ...
        D_loss = (D_loss_real + D_loss_fake) / float(config['num_D_accumulations'])
        D_loss.backward()
        D_loss_real_bar, D_loss_fake_bar = eval('losses.' + config['d_loss'])(D_fake_bar, D_real_bar)
        D_loss_bar = (D_loss_real_bar + D_loss_fake_bar) / float(config['num_D_accumulations'])
        D_loss_bar.backward()
        counter += 1
      ...
      summary = {'D/D_loss_real': torch.mean(D_loss_real),
               ...
               'D/real_scores': torch.mean(D_real),
               'D/fake_scores': torch.mean(D_fake),
               'D_bar/real_scores_bar': torch.mean(D_real_bar),
               'D_bar/fake_scores_bar': torch.mean(D_fake_bar)}
      writer.add_scalars('D', summary, global_steps)
      ...
      #else:
      #    D.optim.step()
      # Update the parameters of Dbar and D
      D.optim.step()
      mask.step(explore_mode='D')
...
def GAN_training_compression_update_function(G, D, D_bar, GD, z_, y_, ema, state_dict, config, writer_dict):
  def train(x, y):
    G.optim.zero_grad()
    D.optim.zero_grad()
    # How many chunks to split x and y into?
    ...
    for step_index in range(config['num_D_steps']):
      ...
      summary = {'D/D_loss_real': torch.mean(D_loss_real),
               ...
               'D/real_scores': torch.mean(D_real),
               'D/fake_scores': torch.mean(D_fake),
               'D_bar/real_scores': torch.mean(D_real_bar),
               'D_bar/fake_scores': torch.mean(D_fake_bar)}
      writer.add_scalars('D', summary, global_steps)
      D.optim.step()
      D_bar.optim.step()
