...
def run(config):
  ...
  # Load D from pretrained sparse models, and initialize the weights of embedding.
  D.load_state_dict(torch.load(config['pretrain_path']))
  #D.optim.load_state_dict(torch.load(os.path.join(config['sparse_model'], 'D_optim_best3.pth')))
  ...
  # sparsify GAN models
  mask = None
  if config['sparse_G'] or config['sparse_D']:
    decay = CosineDecay(config['death_rate'], int(len(loaders[0]) * (config['num_epochs']) * config['multiplier']))
    mask = Masking(G.optim if config['sparse_G'] else False, D.optim if config['sparse_D'] else False, death_rate_decay=decay, **config)
  ...
  if config['which_train_fn'] == 'GAN':
    train = train_fns.GAN_training_sparse_function(mask, G, D, GD, z_, y_, 
