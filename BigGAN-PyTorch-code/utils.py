...
def prepare_parser():
  ...
  parser.add_argument(
    '--pretrain_path', type=str, default='',
    help='Path of pretrained discriminator, for compression (default: %(default)s)')
  parser.add_argument(
    '--sparse_model', type=str, default='',
    help='Path of pretrained model, for compression (default: %(default)s)')
  parser.add_argument(
    '--G_pretrain_path', type=str, default='',
    help='Path of pretrained generator, for compression (default: %(default)s)')
  parser.add_argument(
  ...
  parser.add_argument(
    '--G_arch_lr', type=float, default=3e-4,
    help='Search: Learning rate to use for optimizing the architecture of Generator (default: %(default)s)')
  parser.add_argument(
  ...
  parser.add_argument(
    '--G_arch_B1', type=float, default=0.5,
    help='Search: Beta1 to use for arch Generator (default: %(default)s)')
  parser.add_argument(
    '--G_arch_B2', type=float, default=0.999,
    help='Search: Beta2 to use for arch Generator (default: %(default)s)')
  parser.add_argument(
    '--arch_weight_decay', type=float, default=1e-3,
    help='Search: Weight decay to use for arch Generator (default: %(default)s)')
  parser.add_argument(
  ...
  ### Loss functions ###
  parser.add_argument(
    '--d_loss', type=str, default='', required=True,
    help='The name of the loss function of the discriminator'
    )
  parser.add_argument(
    '--g_loss', type=str, default='', required=True,
    help='The name of the loss function of the generator'
  )
  ...
  parser.add_argument(
    '--inner_steps', type=int, default=20,
    help='Search: Number of steps to optimize weight parameters for (default: %(default)s)')
  parser.add_argument(
    '--outer_steps', type=int, default=20,
    help='Search: Number of steps to optimize architecture parameters for (default: %(default)s)')
  parser.add_argument(
    '--worst_steps', type=int, default=20,
    help='Search: Number of steps to find worst generator and worst discriminator for (default: %(default)s)')
  parser.add_argument(
  ...
  parser.add_argument(
    '--weights_root', type=str, default='/apdcephfs/share_1367250/yuesongtian/BigGAN_results/weights',
    help='Default location to store weights (default: %(default)s)')
  parser.add_argument(
    '--logs_root', type=str, default='/apdcephfs/share_1367250/yuesongtian/BigGAN_results/logs',
    help='Default location to store logs (default: %(default)s)')
  parser.add_argument(
    '--samples_root', type=str, default='/apdcephfs/share_1367250/yuesongtian/BigGAN_results/samples',
...
# Return train loader and val loader for searching.
def get_data_loaders_search(dataset, data_root=None, augment=False, batch_size=64, 
                     num_workers=8, shuffle=True, load_in_mem=False, hdf5=False,
                     pin_memory=True, drop_last=True, start_train_itr=0, start_val_itr=0,
                     num_train_epochs=500, num_val_epochs=500, use_multiepoch_sampler=False,
                     **kwargs):
  ...
  total_set = which_dataset(root=data_root, transform=train_transform,
                            load_in_mem=load_in_mem, **dataset_kwargs)
  mid = int(len(total_set) * 0.5)
  sets = torch.utils.data.random_split(total_set, [mid, len(total_set)-mid])
  ...
  if use_multiepoch_sampler:
    print('Using multiepoch sampler from start_itr %d...' % start_train_itr)
    loader_kwargs = {'num_workers': num_workers, 'pin_memory': pin_memory}
    train_sampler = MultiEpochSampler(train_set, num_train_epochs, start_train_itr, batch_size)
    train_loader = DataLoader(train_set, batch_size=batch_size,
                              sampler=train_sampler, **loader_kwargs)
    val_sampler = MultiEpochSampler(val_set, num_val_epochs, start_val_itr, batch_size)
    val_loader = DataLoader(train_set, batch_size=batch_size,
                              sampler=val_sampler, **loader_kwargs)
    print(f'length val_set is {len(val_set)}, length train_set is {(len(train_set))}')
    print(f'length val_loader is {len(val_loader)}, length train_loader is {len(train_loader)}')
  else:
    loader_kwargs = {'num_workers': num_workers, 'pin_memory': pin_memory,
...
# Convenience function to centralize all data loaders
def get_data_loaders_parallel(dataset, data_root=None, augment=False, batch_size=64, 
                     num_workers=8, shuffle=True, load_in_mem=False, hdf5=False,
...
# Convenience function to centralize all data loaders, without pre-processing
def get_data_loaders_woPreprocess(dataset, data_root=None, augment=False, batch_size=64, 
                     num_workers=8, shuffle=True, load_in_mem=False, hdf5=False,
                     pin_memory=True, drop_last=True, start_itr=0,
                     num_epochs=500, use_multiepoch_sampler=False,
                     **kwargs):
  ...
  # For calculating the statical feature of real datasets, do not transform the dataset
  train_transform = [CenterCropLongEdge(), transforms.Resize(image_size)]
  train_transform = transforms.Compose(train_transform + [
                     transforms.ToTensor(),
                     transforms.Normalize(norm_mean, norm_std)])
  ...
  #train_set = torchvision.datasets.ImageNet(root=data_root, split='train')
  ...
  def __init__(self, source, target, decay=0.9999, start_itr=0):
    ...
    with torch.no_grad():
      for key in self.source_dict:
        # For Generator_student with conv_1x1 to transfrom the feature maps to be distilled.
...
def save_search_weights(G, D, state_dict, weights_root, experiment_name, 
  ...
  torch.save(G.weight_optim.state_dict(), 
              '%s/%s.pth' % (root, join_strings('_', ['G_weight_optim', name_suffix])))
  torch.save(G.arch_optim.state_dict(), 
              '%s/%s.pth' % (root, join_strings('_', ['G_arch_optim', name_suffix])))
...
def load_weights(G, D, state_dict, weights_root, experiment_name, 
  ...
  root = '/'.join([weights_root, experiment_name])
  if not os.path.exists(os.path.join(weights_root, experiment_name)):
      os.mkdir(os.path.join(weights_root, experiment_name))
  print(f'loading path is {os.path.join(root, "%s.pth" % (join_strings("_", ["G", name_suffix])))}')
  if not os.path.exists(os.path.join(root, '%s.pth' % (join_strings('_', ['G', name_suffix])))):
      return
  if name_suffix:
...
# Load a model's weights, optimizer, and the state_dict
def load_weights_search(G, D, state_dict, weights_root, experiment_name, 
                 name_suffix=None, G_ema=None, strict=True, load_optim=True):
  root = '/'.join([weights_root, experiment_name])
  if not os.path.exists(os.path.join(weights_root, experiment_name)):
      os.mkdir(os.path.join(weights_root, experiment_name))
  ...
  if G is not None:
    G.load_state_dict(
      ...
      strict=strict)
    if load_optim:
      G.weight_optim.load_state_dict(
        torch.load('%s/%s.pth' % (root, join_strings('_', ['G_weight_optim', name_suffix]))))
      G.arch_optim.load_state_dict(
        torch.load('%s/%s.pth' % (root, join_strings('_', ['G_arch_optim', name_suffix]))))
  if D is not None:
    D.load_state_dict(
      torch.load('%s/%s.pth' % (root, join_strings('_', ['D', name_suffix]))),
      strict=strict)
