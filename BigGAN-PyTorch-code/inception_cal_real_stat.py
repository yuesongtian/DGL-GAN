...
MODEL_DIR = '/apdcephfs/share_1367250/yuesongtian/pretrained_models'
DATA_URL = '/apdcephfs/share_1367250/yuesongtian/pretrained_models/inception-2015-12-05.tgz'
softmax = None
...
def run(config):
  ...
  def get_activations_loaders(loader, sess, batch_size=50, verbose=False):
    """Calculates the activations of the pool_3 layer for all images.

    Params:
    -- loader      : The data loader of real dataset.
    -- sess        : current session
    -- batch_size  : the images numpy array is split into batches with batch size
                     batch_size. A reasonable batch size depends on the disposable hardware.
    -- verbose    : If set to True and parameter out_step is given, the number of calculated
    ...
    for i, (x, y) in enumerate(tqdm(loader)):
        ...
        print(f'img is {torch.max(x)}')
        img = x.permute(0,2,3,1).cpu().detach().numpy()
        pred = sess.run(inception_layer, {'ExpandDims:0': img})
        print(f'pred shape is {pred.shape}, start is {start}, end is {end}, pred_arr is {pred_arr.shape}, real_img is {x.shape}, d0 is {d0}')
        pred_arr[start:end] = pred.reshape(batch_size, -1)
    if verbose:
        print(" done")
    return pred_arr
  ...
  # Get data loaders
  loaders = utils.get_data_loaders_woPreprocess('I128', data_root="/apdcephfs/share_1367250/yuesongtian/imagenet2012/ImageNet/train", augment=False, batch_size=500,
                   num_workers=8, shuffle=True, load_in_mem=False, hdf5=False,
                   pin_memory=True, drop_last=True, start_itr=0,
                   num_epochs=1, use_multiepoch_sampler=False)

  # Calculate features of real dataset
  with tf.Session(config=config_tf) as sess:
    pred = get_activations_loaders(loaders[0], sess, batch_size=500)
  mu = np.mean(pred, axis=0)
  sigma = np.cov(pred, rowvar=False)
  
  # Save features of real dataset
  np.savez('/apdcephfs/share_1367250/yuesongtian/pretrained_models/fid_stat/I128_train_stat.npz', mu=mu, sigma=sigma)
