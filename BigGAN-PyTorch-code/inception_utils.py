...
def load_inception_net(parallel=False):
  source = torch.load('/apdcephfs/share_1367250/yuesongtian/inception_model/inception_v3_google-1a9a5a14.pth')
...
def prepare_inception_metrics(dataset, parallel, no_fid=False, data_root=None):
  ...
  print(f'inception_moments is {os.path.join(data_root, dataset+"_inception_moments.npz")}')
  if 'I128' in dataset:
    data_mu = np.load(os.path.join(data_root, 'ft_local', dataset+'_inception_moments.npz'))['mu']
    data_sigma = np.load(os.path.join(data_root, 'ft_local', dataset+'_inception_moments.npz'))['sigma']
  else:
    data_mu = np.load(os.path.join(data_root, dataset+'_inception_moments.npz'))['mu']
    data_sigma = np.load(os.path.join(data_root, dataset+'_inception_moments.npz'))['sigma']
  # Load network
