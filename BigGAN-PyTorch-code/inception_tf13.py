...
def run(config):
  ...
  def get_activations_loaders(loader, sess, batch_size=50, verbose=False):
    ...
    for i, (x, y) in enumerate(tqdm(loader)):
        print(f'real image shape is {x.shape}')
        if verbose:
            print("\rPropagating batch %d/%d" % (i + 1, n_batches), end="", flush=True)
        start = i * batch_size
        end = start + batch_size
        batch = images[start:end]
        pred = sess.run(inception_layer, {'ExpandDims:0': x})
        pred_arr[start:end] = pred.reshape(batch_size, -1)
    if verbose:
        print(" done")
    return pred_arr
  ...
  fname = '/apdcephfs/share_1367250/yuesongtian/pretrained_models/fid_stat/I128_train_stat.npz'
